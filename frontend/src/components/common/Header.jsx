...
const Header = () => {
  ...
  const navLink = (path, label) => (
    <Link
      to={path}
      className={`relative text-sm font-semibold transition-colors duration-200 link-underline ${
        isActive(path)
  ...
  return (
    <header
      className={`sticky top-0 z-50 transition-all duration-300 ${
        scrolled ? 'header-glass header-scrolled' : 'header-glass'
      }`}
    >
              ...
              <div className="relative">
                ...
                {isUserMenuOpen && (
                  ...
                  >
                    ...
                    {isAdmin() && (
                      <Link to="/admin" className="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-300 hover:bg-white/5 hover:text-[#FF8C00] transition-colors">
                    ...
                    <Link to="/my-orders" className="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-300 hover:bg-white/5 hover:text-white transition-colors">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <nav className="md:hidden py-4 border-t border-white/5 animate-slideInDown">
            {[['/', 'Home'], ['/products', 'Products'], ['/about', 'About'], ['/contact', 'Contact']].map(([path, label]) => (
              <Link
                key={path}
                to={path}
                className={`flex items-center gap-3 py-3 px-2 rounded-xl text-sm font-semibold transition-colors ${
                  isActive(path) ? 'text-[#FF8C00]' : 'text-gray-400 hover:text-white hover:bg-white/3'
                }`}
